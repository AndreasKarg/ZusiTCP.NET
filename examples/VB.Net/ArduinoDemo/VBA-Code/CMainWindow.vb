...
Public Class CMainWindow
    ...
    ''' <summary>
    ''' Der Konstrukor. Er wird noch vor dem Start aufgerufen und sorgt daf�r, dass unser Form so aussieht, wie wir es uns vorstellen.
    ''' </summary>
    Public Sub New()
        InitializeComponent()
        ...
        'Die ComboBox mit Eintr�gen f�llen
        AddCom2Combo(CbCom)
    End Sub
    ''' <summary>
    ...
    End Sub
    ''' <summary>
    ''' Wird ausgel�st, wenn in der TCPConnection ein Fehler aufgetreten ist.
    ''' </summary>
        ...
        If Not Serial.IsOpen Then
            ...
            If Serial.BytesToRead > 0 Then
                If Not Serial.ReadLine() = "I am a drivers desk" & System.Text.Encoding.UTF8.GetString(New Byte() {13}) Then
                    System.Windows.Forms.MessageBox.Show("An dem gew�hlten Port scheint das falsche Ger�t angeschlossen zu sein!")
                    Serial.Close()
                    ComCon.Text = "Verbinden"
                End If
            Else
                System.Windows.Forms.MessageBox.Show("An dem gew�hlten Port scheint kein Ger�t angeschlossen zu sein!")
                Serial.Close()
                ComCon.Text = "Verbinden"
            End If
        Else
    ...
    ''' <summary>
    ''' Wird ausgel�st, wenn ein Boolean-Wert empfangen wurde.
    ''' </summary>
    Private Sub MyTCPConnection_BoolReceived(ByVal sender As Object, ByVal data As Zusi_Datenausgabe.DataSet(Of Boolean)) Handles MyTCPConnection.BoolReceived
        ...
        End Select
        If Serial.IsOpen Then 'Wenn wir mit unserer Hadware verbunden sind, k�nnen wir die Daten gleich senden.
    ...
    End Sub
    ''' <summary>
    ''' Wird ausgel�st, wenn ein Single-Wert empfangen wurde.
    ''' </summary>
